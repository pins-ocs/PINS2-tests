...
mechatronix do |data|
  ...
  pars, table = Utils::read_from_table(File.expand_path('../weights_ax_prev.txt', __FILE__))
  #p table 
  
  # p "Data read correctly and trajectory built"
  data.SplineSetWeightsAxPrev =
    {
      #:spline_type  => [ "quintic", "quintic" ],
      :spline_type  => ["linear", "linear"],
      :headers      => ["w_ax_prev", "w_ax_new"],
      :xdata         => table["zeta"], #.map {|x|x*Math::PI/180.0},
      :ydata         => [table["w_ax_prev"], 
                         table["w_ax_new"], ],
    }
