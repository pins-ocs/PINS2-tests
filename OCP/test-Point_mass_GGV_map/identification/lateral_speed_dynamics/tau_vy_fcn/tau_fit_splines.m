%-------------------------------------------------------------------------------
%% Fit tau_vy vs u using cubic splines
%-------------------------------------------------------------------------------

% Load the data
u_data   = load('u_fit.mat').speed_vect_interp';
tau_data = load('tau_fit.mat').tau_v_opt_interp';

% Filter the data by fitting with a smoothing spline
f = fit(u_data, tau_data, 'smoothingspline', 'SmoothingParam', 1e-3);
...
fileID = fopen('tau_vy_vs_u_spline.txt','w');
