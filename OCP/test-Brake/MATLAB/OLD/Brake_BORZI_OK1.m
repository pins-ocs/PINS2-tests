%
%
%  target: x(end) + int_0^1 (-mu*log(cos(pi/2*a))) dt
%
%  ODE:  x' = T v
%        v' = T a
%        T' = 0
...
%  Hamiltonian
%
%  H = -mu*log(cos(pi/2*a)) + lambda1 T v + lambda2 T a [ + T*(epsilon/2)*(a-A)^2 ]
%
%  co-equation
%
%  H_x = 0
%  H_v = lambda1 T
%  H_T = lambda1 v + lambda2 a +  [ + (epsilon/2)*(a-A)^2 ]
%
%  control (indirect)
%
%  H_a   = -mu*tan(pi/2*a)*pi/2 + lambda2 T
%  /T/mu -> -tan(pi/2*a) + 2*T*lambda2/(mu*pi) --> a = (2/pi)*arctan(2*T*lambda2/(mu*pi))
%
%  control (Borzi)
%
%  K   = -mu*log(cos(pi/2*a)) + lambda2 T + lambda2 T a + T*(epsilon/2)*(a-A)^2
%  K_a = -mu*tan(pi/2*a)*pi/2 + lambda2 T + T*epsilon*(a - A)
%
%  a - (mu/epsilon)*(pi/2)*tan(pi/2*a) =  A - T*lambda2/epsilon
%
